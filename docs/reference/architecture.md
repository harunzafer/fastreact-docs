...
This document explains how [FastReact](https://fastreact.dev) is structured and the reasoning behind key architectural decisions.

## 1. The Monorepo Structure

FastReact has four parts:
...
FastReact uses a monorepo because it's a tightly coupled fullstack application where the backend and frontend are designed to work together. When the backend API changes, the frontend needs to change with it - keeping them in separate repositories would mean managing dependencies, versioning, and synchronization across repos. A monorepo allows atomic commits that update the database schema, backend logic, and frontend UI simultaneously, ensuring the entire system stays in sync. This simplifies development with a single clone and unified tooling.

### Design Philosophy: Minimal Dependencies, Maximum Flexibility

FastReact intentionally avoids many popular libraries and frameworks that other starter kits include. Instead of bundling heavy abstractions, it sticks to proven, essential tools with minimal dependencies.

**Why stay lean?** Adding a library later is straightforward - removing one that's baked into the starter kit is painful. As a starter kit, FastReact's job is to provide a solid foundation, not to make architectural decisions for you. This approach makes the codebase highly customizable. Build exactly what you need, add libraries as your requirements become clear, and maintain full control over your architecture.
...
In FastReact, all objects are wired up in one place (`app/config/container.py`):
...
FastReact ships two React Router projects that render differently on purpose:

| | App (`frontend/`) | Landing (`landing/`) |
|---|---|---|
| **Mode** | SPA (`ssr: false`) | SSG (`ssr: false` + `prerender: true`) |
| **Build output** | Static files in `build/client` with an `index.html` fallback | Static HTML per route in `build/client`, with real content |
...
Both projects build to plain static files, so **the only server in any deployment is the FastAPI backend**. There is no Node tier to run, scale, or pay for: `react-router build` produces `build/client`, and that directory is the whole deployment. Deep links like `/settings` still work on a static host because the deploy configs shipped with the kit (`vercel.json`, `staticwebapp.config.json`) rewrite unknown paths to the SPA fallback.

Prerendering freezes the landing's content at build time. Editing marketing copy means rebuild and redeploy, and `VITE_*` variables are baked in during the build (set them in CI, not in your host's runtime settings).

**If your landing outgrows static.** The landing's routes are server-rendered at build time, so the code stays fully server-renderable. If you later need per-request rendering (personalization, instant-publish content), set `ssr: true` in `landing/react-router.config.ts`, add `@react-router/serve` back as a dependency, and run `react-router-serve ./build/server/index.js`. That is a config change, not a rewrite. Forms like the newsletter signup never need it: point them at a FastAPI endpoint from the client.
...
FastReact uses domain exceptions that inherit from `BaseAppException`. Each exception knows its own HTTP status code, error code, and message format. A global error handler middleware automatically converts these to JSON responses.

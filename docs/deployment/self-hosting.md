...
`frontend/` and `landing/` build to plain static files in `build/client`, and `VITE_*` variables are baked in at build time, so set them before `npm run build`. There are two ways to serve the output; Option A is simpler.
...
```bash
sudo mkdir -p /var/www/fastreact/app /var/www/fastreact/landing

cd frontend
npm ci
VITE_API_BASE_URL=https://api.yourdomain.com npm run build
sudo cp -r build/client/. /var/www/fastreact/app/

cd ../landing
npm ci
npm run build
sudo cp -r build/client/. /var/www/fastreact/landing/
```

Rebuild and re-copy whenever the code or any `VITE_*` value changes. The landing is prerendered, so marketing content edits also need a rebuild.
...
`VITE_API_BASE_URL` must be a **build arg** (under `build.args` in your compose service), not a runtime variable: a static build cannot read the environment after it is built. Repeat for `landing/`, changing the nginx line to `try_files $uri $uri/ =404;` (the landing is prerendered HTML, not an SPA). With this option, the nginx blocks below `proxy_pass` to the containers instead of serving files.
...
```nginx
# /etc/nginx/sites-available/fastreact
server {
...
server {
    ...
    root /var/www/fastreact/app;
    try_files $uri /index.html;
}

# Landing (prerendered): serve the per-route HTML
server {
    server_name yourdomain.com;
    root /var/www/fastreact/landing;
...
```bash
# Backup database
docker-compose exec db pg_dump -U postgres fastreact > backup.sql

# Backup volumes
docker run --rm -v fastreact_postgres_data:/data -v $(pwd):/backup ubuntu tar czf /backup/postgres_backup.tar.gz /data
```
...
(cd frontend && npm run build) && sudo cp -r frontend/build/client/. /var/www/fastreact/app/
(cd landing && npm run build) && sudo cp -r landing/build/client/. /var/www/fastreact/landing/
```

---
description: "Get FastReact running in under 10 minutes. Complete setup guide for FastAPI backend, React Router v7 frontend, PostgreSQL database, and authentication system."
keywords: "fastreact setup, getting started, fastapi setup, react installation, postgresql, docker, development environment"
---

# Getting Started

This guide will get you up and running with [FastReact](https://fastreact.dev) in under 10 minutes. By the end, you'll have a fully functional SaaS application running locally with authentication, user management, and payment processing.
...
git clone git@github.com:harunzafer/fastreact.git my-project

# Option 2: HTTPS (requires Personal Access Token)
git clone https://github.com/harunzafer/fastreact.git my-project

cd my-project

# Remove the original remote (prevents accidental pushes to FastReact repo)
...
1. **Clone your repository** — not the FastReact template
...
The backend environment variables are in `backend/.env` using the `FR_` prefix. These are loaded using [Pydantic Settings](https://docs.pydantic.dev/latest/concepts/pydantic_settings/). Here are the core variables you should be aware of:

- `FR_APP_NAME`: Your application name
- `FR_MODE`: b2c or b2b (see [B2B Mode](b2b-mode.md) for details)
- `FR_ENVIRONMENT`: dev, beta, or prod
- `FR_DB_URL` / `FR_DB_SCHEMA`: Database connection
- `FR_BASE_WEB_URL` / `FR_BASE_API_URL`: Frontend and backend URLs
- `FR_JWT_SECRET_KEY`: OAuth state validation (auto-generated)
- `FR_CRON_SECRET`: Cron job authentication (auto-generated)
...
Now that you have FastReact running:

- **[Development Guide](development.md)** - Learn common development workflows
- **[Architecture Overview](architecture-overview.md)** - Understand how FastReact is built

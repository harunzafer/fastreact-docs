---
description: "FastReact security defaults — Argon2 password hashing, hashed session tokens, role-based access control, OAuth CSRF protection, AI spend caps, per-IP rate limiting, and per-environment cookie and CORS hardening."
keywords: "fastreact security, argon2, session security, rbac, csrf, cors, secure cookies, ai budget cap, rate limiting, saas security"
---
...
To stay lean, FastReact does **not** bundle security-header middleware (CSP/HSTS). Add it when your threat model calls for it, e.g. a middleware that sets security headers.

---
description: "FastReact security: audited defaults with Argon2 password hashing, hashed session tokens, role-based access control, hardened Google sign-in, AI spend caps, per-IP rate limiting, HTTP security headers, dependency monitoring, and per-environment cookie and CORS hardening."
keywords: "fastreact security, argon2, session security, rbac, csrf, cors, secure cookies, ai budget cap, rate limiting, security headers, content security policy, saas security"
---
...
FastReact protects API responses by default and provides a ready-to-use header policy for your frontend.

### API: already protected

FastReact adds these headers to every API response, including errors:
...
!!! note "Why the policy allows inline styles"
    ...
    The policy sets `style-src 'self' 'unsafe-inline'`, which permits inline styles the build or UI components may emit. This applies to styles only. Scripts stay limited to your own domain, which is where the real risk lies.

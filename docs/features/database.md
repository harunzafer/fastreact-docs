---
description: "FastReact database — multi-tenant PostgreSQL schema, Sqitch migrations, and the db_config connection layer."
keywords: "fastreact database, postgresql, sqitch migrations, multi-tenant schema, raw sql, database versioning, fastapi postgres"
---
...
The `sqitch.sh` wrapper runs Sqitch via the official Docker image (no local Sqitch install needed) and adds FastReact-specific safety: per-environment database URLs (dev/beta/gamma/prod/test), a check that every migration is wrapped in `BEGIN;`/`COMMIT;`, revert protection (requires `--to`), and `.env` loading per environment.

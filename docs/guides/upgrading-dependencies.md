---
description: "Keep a FastReact project's dependencies current and secure — the Dependabot policy, backend (uv) and frontend (npm) upgrade workflows, and major-version upgrades."
keywords: "fastreact dependencies, dependabot, uv upgrade, npm update, dependency management, major version upgrade"
---
...
FastReact ships a [Dependabot config](https://github.com/) (`.github/dependabot.yml`) so routine upgrades are automated and reviewable:
...
- Frontend smoke tests live in `frontend/app/tests/smoke/` and run with `npm run test`.

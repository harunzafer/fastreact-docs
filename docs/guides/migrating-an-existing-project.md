---
description: "Migrate an existing app onto FastReact with your AI agent: plan the migration, break it into small reviewable tasks, and port feature by feature."
keywords: "fastreact migration, migrate existing project, port app to fastreact, ai agent migration, rebuild app on starter kit"
---

# Migrating an Existing Project

You have an app that works. Maybe you built it with AI, maybe by hand, and now you want it standing on FastReact's foundation instead of hardening your own. This guide is the process we used to migrate a production app onto FastReact ourselves, driven by an AI agent the whole way.

The direction matters: you do not port FastReact into your project. You move your project onto a fresh FastReact clone, where auth, billing, organizations, and email already work, so the only things your agent rebuilds are your own features.
...
- A FastReact clone, set up and running locally (see [Project Setup](project-setup.md))
- Your existing project available on the same machine
- An AI agent (Claude Code, Cursor, or similar) running **in the FastReact clone**, not in your old project
...
```text
I want to migrate an existing project onto this FastReact codebase.
...
2. Already covered: which of those features FastReact already provides
...
3. To port: the features that must be rebuilt here, each mapped to the
   FastReact layers it will touch (schema, repository, service, route,
   frontend).
...
- **Did it map features to FastReact's built-ins correctly?** A common miss is rebuilding something FastReact already ships, like password reset or invitations.
...
2. **Deploy** the FastReact app alongside the old one (see [Deployment](../deployment/index.md)) and use it with real accounts for a few days.

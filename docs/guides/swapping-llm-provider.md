---
description: "Swap FastReact's OpenAI integration for Claude (Anthropic), Gemini, or any provider by implementing the LLMClient interface, with a complete drop-in Claude recipe."
keywords: "fastreact llm provider, swap openai, claude anthropic, gemini, litellm, llmclient, ai saas starter kit"
---

# Swapping the LLM Provider

FastReact ships OpenAI as the default LLM, but no route, service, or billing code ever imports a provider SDK directly. Everything depends on a small `LLMClient` seam (see [AI Integration](../features/ai.md#the-llmclient-interface)). To switch providers you write one class implementing that interface and repoint the dependency-injection container at it. Nothing else in the backend changes.
